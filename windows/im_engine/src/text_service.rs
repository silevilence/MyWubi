...
impl TextService {
    ...
    pub fn new(dict: Arc<Dictionary>, page_size: usize, auto_commit_unique: bool) -> Self {
    ...
    /// 从 [`Config`] 选择默认参数。
    pub fn from_config(dict: Arc<Dictionary>, cfg: &Config) -> Self {
        Self::new(
            dict,
    ...
    fn apply_transition(&self, t: Transition) -> BOOL {
        match t {
            ...
            Transition::Commit(text) => {
                // 上屏文本暂时通过日志输出，真实插入将在
                // “基于 Slint 的候选框”阶段通过 ITfEditSession 完成。
