...
- [x] **码表 YAML 头与配置扩展**
    - [x] 码表文件格式调整：开头新增可选 YAML 头（`---` 分隔），用于存放码表级配置
    - [x] YAML 头解析：`core_engine` 码表加载时识别并解析，无 YAML 头则全部使用默认值
    - [x] 支持的配置项（均有默认值）：
        - `wildcard_key`：万能键字符，单个编码字符，默认空（无万能键）
        - `charset`：编码字符集，字符串形式列出所有可用编码字符（含万能键），默认 `abcdefghijklmnopqrstuvwxyz`
    - [x] 校验逻辑：万能键字符必须包含在 `charset` 中；码表正文编码不得出现万能键字符

- [x] **万能键（通配符）**
    - [x] 万能键行为由码表 YAML 头 `wildcard_key` 驱动，`config.toml` 不再持有该配置
    - [x] `settings.exe` 常规面板显示当前码表的万能键状态（只读提示，编辑移至码表管理面板）
    - [x] 引擎检索层根据码表 `wildcard_key` 动态绑定通配字符，匹配任意编码位置，候选列表实时刷新

- [x] **码表管理面板**
    - [x] 在 `settings.exe` 新增码表管理面板，复用用户词库的列表交互模式
    - [x] 支持编辑码表 YAML 头配置（万能键、编码字符集等），带格式校验与即时预览
    - [x] 虚拟列表渲染，支持百万级词条流畅滚动与搜索
    - [x] 支持按字词/编码模糊搜索、在线修改权重与编码
    - [x] 支持另存为自定义码表（输入名称，自动保存至码表目录）
    - [x] 码表验证功能：检查所有词条编码是否均在 `charset` 范围内且不含万能键字符，报告违规条目

- [x] **快速选择第二第三候选**
    - [x] **配置层**：在 `[hotkey]` 新增 `select_second`（默认 `"semicolon"`）和 `select_third`（默认 `"quote"`）两个可配置键位，`config.rs` 的 `Hotkey` 结构体同步扩展
    - [x] **按键映射层**：`key_filter::translate()` 新增"候选快速选择"逻辑——当输入状态为 Selecting 且按下对应配置键时，发射 `InputEvent::Select(2)` / `Select(3)`；无候选时将该键作为普通字符透传
    - [x] **热重载支持**：配置变更后通过 `ArcSwap` 原子替换 `RuntimeSnapshot`，快速选择键位即时生效
    - [x] **设置界面**：`settings.exe` 常规面板新增第二/第三候选快速选择键的下拉配置项，复用现有翻页键的键位选择交互模式
