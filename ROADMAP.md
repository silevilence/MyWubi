...
- [ ] **万能键（通配符）**
    - [ ] 万能键行为由码表 YAML 头 `wildcard_key` 驱动，`config.toml` 不再持有该配置
    - [ ] `settings.exe` 常规面板显示当前码表的万能键状态（只读提示，编辑移至码表管理面板）
    - [ ] 引擎检索层根据码表 `wildcard_key` 动态绑定通配字符，匹配任意编码位置，候选列表实时刷新

- [ ] **码表管理面板**
    - [ ] 在 `settings.exe` 新增码表管理面板，复用用户词库的列表交互模式
    - [ ] 支持编辑码表 YAML 头配置（万能键、编码字符集等），带格式校验与即时预览
    - [ ] 虚拟列表渲染，支持百万级词条流畅滚动与搜索
    - [ ] 支持按字词/编码模糊搜索、在线修改权重与编码
    - [ ] 支持另存为自定义码表（输入名称，自动保存至码表目录）
    - [ ] 码表验证功能：检查所有词条编码是否均在 `charset` 范围内且不含万能键字符，报告违规条目
